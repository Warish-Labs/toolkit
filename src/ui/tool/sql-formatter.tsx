...
import { formatSql, minifySql } from "@/src/logic/sql-formatter";
...
export function SqlFormatterTool() {
  ...
  const handleFormat = () => {
    setError("");
    try {
      const res = formatSql(sqlInput, indentSize);
      setResult(res);
    } catch (e) {
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <textarea
          ...
          placeholder="e.g. select id, name, description from tools where category = 'calculators' order by name asc"
          value={sqlInput}
          onChange={(e) => { setSqlInput(e.target.value); setError(""); }}
        />
      </div>
